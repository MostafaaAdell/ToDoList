...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [_modelNetwork filterDataAccordingToStatus:1 withPredicate:@"status == %d"];
        ...
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleDefault
         ...
         UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"Ok" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
             if (editingStyle == UITableViewCellEditingStyleDelete) {
                 [self->_modelNetwork removeCertainTaskWithIndex:(int)indexPath.row WithSection:(int)indexPath.section withStatus:1 withPredicate:@"status == %d"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    edittaskController.modelNetwork.flag = 1;
